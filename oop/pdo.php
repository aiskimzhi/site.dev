...
if (isset($_GET['action']) and $_GET['action'] == 'delete') {
	$delete = $pdo->prepare("DELETE FROM table_1 WHERE id = :id");
	$delete->execute(array('id' => $_GET['id']));
}

//Выбираем данные для апдейта
if (isset ($_GET['action']) and $_GET['action'] == 'update') {
	$select = $pdo->prepare("SELECT 'surname', 'name', 'fname' FROM table_1 WHERE id = :id");
...
if (isset($_GET['action'])
    ...
    $update = $pdo->prepare("UPDATE table_1 SET surname = :surname, name = :name, fname = :fname WHERE id = :id");
    $update->execute([
        'id' => $_GET['id'],
...
if (isset($_POST['send'])
    ...
    $insert = $pdo->prepare("INSERT INTO table_1 (surname, name, fname) VALUES (:surname, :name, :fname)");
    $insert->execute(array('surname' => $_POST['surname'], 'name' => $_POST['name'], 'fname' => $_POST['fname']));
    $valSurname = '';
    $valName = '';
    $valFname = '';
}
...
$selectAll = $pdo->prepare("SELECT * FROM table_1");
$selectAll->execute();
$all = $selectAll->fetchAll();

//Создаю заголовки таблицы и добавляю их в массив таблицы
$head = array('id' => 'ID', 'surname' => 'SURNAME', 'name' => 'NAME', 'fname' => "FATHER'S NAME", 'del' => 'DELETE', 'up' => 'UPDATE');
array_unshift ($all, $head);

//создаю ссылки для апдейта и делита и добавляю их в массив таблицы
for ($d = 1; $d < count($all); $d++) {
	$del[$d] = '<a href="http://site.dev/oop/pdo.php?action=delete&id=' . $all[$d]['id'] . '">DELETE</a>';
	$up[$d] = '<a href="http://site.dev/oop/pdo.php?action=update&id=' . $all[$d]['id'] . '">UPDATE</a>';
}
...
if (isset($_POST['send']) and empty($_POST['name'])) {
...
<div>
    <form action="" method="post">
        ...
        <div  class="form-group <?php echo !empty($label['name']) ? 'has-error' : ''; ?>">
            ...
            <input type="text" name="name" class="form-control" id="nam" value="<?php echo $valName; ?>">
